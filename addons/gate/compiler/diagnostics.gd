...
class GateDiag extends RefCounted:
	var level: int
	var message: String
	var hint: String
	var file: String
...
func _record(level: int, msg: String, line: int, col: int, hint: String) -> void:
	...
	if _seen.has(key):
		return
	_seen[key] = true
	items.append(GateDiag.new(level, msg, file, line, col, hint))
...
func sort_by_position() -> void:
	items.sort_custom(func(a: GateDiag, b: GateDiag) -> bool:
		if a.line != b.line:
