...
func _expr(e) -> String:
	...
	if e is GateAST.GateLiteral:
		return (e as GateAST.GateLiteral).raw
	if e is GateAST.GateIdent:
		var iname: String = (e as GateAST.GateIdent).name
		...
		if _soa.has(iname):
			...
			return "null"
		var gb: String = (e as GateAST.GateIdent).generic_base
		if gb != "":
	...
	if e is GateAST.GateSelfExpr:
		return "self"
	if e is GateAST.GateNodePathExpr:
		return (e as GateAST.GateNodePathExpr).raw
	if e is GateAST.GateRawExpr:
		return (e as GateAST.GateRawExpr).text
	...
	if e is GateAST.GateUnary:
		...
		if u.operand is GateAST.GateBinary and (u.operand as GateAST.GateBinary).op == "**":
			return "%s(%s)" % [op, _expr(u.operand)]
		if op == "+" and not u.tight and u.operand is GateAST.GateLiteral:
			var raw: String = (u.operand as GateAST.GateLiteral).raw
	...
	if e is GateAST.GateTernary:
		...
		_pending = saved_p
		if t3.if_true is GateAST.GateLambda:
			av = "(%s)" % av
		if t3.if_false is GateAST.GateLambda:
			bv = "(%s)" % bv
		if ap.is_empty() and bp.is_empty():
			if t3.if_false is GateAST.GateTernary:
	...
	if e is GateAST.GateMember and (e as GateAST.GateMember).target is GateAST.GateIdent:
		var sm: GateAST.GateMember = e
		var skey: String = "%s.%s" % [(sm.target as GateAST.GateIdent).name, sm.name]
	...
	if e is GateAST.GateMember:
		...
		if m.target is GateAST.GateIdent and _soa_cursors.has((m.target as GateAST.GateIdent).name):
			var cur: Dictionary = _soa_cursors[(m.target as GateAST.GateIdent).name]
		...
		if m.target is GateAST.GateIndex:
			var mix: GateAST.GateIndex = m.target
	...
	if e is GateAST.GateAwaitExpr:
		return "await " + _expr((e as GateAST.GateAwaitExpr).operand)

	if e is GateAST.GateCastExpr:
		var ce: GateAST.GateCastExpr = e
		...
		if ce.operand is GateAST.GateLambda:
			inner = "(%s)" % inner
		if ce.operand is GateAST.GateCastExpr:
...
func _needs_copy(e) -> bool:
	if e is GateAST.GateCall or e is GateAST.GateObjectInit:
		return false
...
func _emit_postfix_spine(e) -> String:
	...
	while true:
		if node is GateAST.GateMember:
			...
			node = m.target
		elif node is GateAST.GateIndex:
			var ix: GateAST.GateIndex = node
	...
	for st in steps:
		if st is GateAST.GateMember:
			out += ".%s" % (st as GateAST.GateMember).name
		else:
			out += "[%s]" % _expr((st as GateAST.GateIndex).index)
	return out
...
func _emit_binary(b: GateAST.GateBinary) -> String:
	...
	if (op == "and" or op == "or") and b.left is GateAST.GateBinary and (b.left as GateAST.GateBinary).op == op:
		var achain: Array = []
		var anode = b
		while anode is GateAST.GateBinary and (anode as GateAST.GateBinary).op == op:
			achain.push_front(anode)
			anode = (anode as GateAST.GateBinary).left
		if achain.size() > 1:
			var saved_p: PackedStringArray = _pending
			_pending = PackedStringArray()
			var parts: PackedStringArray = PackedStringArray()
			parts.append(_paren_below((achain[0] as GateAST.GateBinary).left, p))
			for n in achain:
				parts.append(_paren_below((n as GateAST.GateBinary).right, p + 1))
	...
	if op != "and" and op != "or" and b.left is GateAST.GateBinary:
		var chain: Array = []
		var node = b
		while node is GateAST.GateBinary:
			var nb: GateAST.GateBinary = node
			if nb.op == "and" or nb.op == "or" or nb.op == "not in":
				break
			var lt2: String = _static_type_of(nb.left)
			if lt2 != "" and _struct_ops.has(lt2):
				break
			if not (nb.left is GateAST.GateBinary):
				break
			var child: GateAST.GateBinary = nb.left
...
func _emit_is(ie: GateAST.GateIsExpr) -> String:
...
func _check_struct_arity(name: String, st: Dictionary, c: GateAST.GateCall) -> void:
...
func _emit_call(c: GateAST.GateCall) -> String:
	if (c.callee is GateAST.GateIdent and (c.callee as GateAST.GateIdent).name == "preload"
			and c.args.size() == 1 and c.args[0] is GateAST.GateLiteral):
		var lit: GateAST.GateLiteral = c.args[0]
		if lit.kind == "string":
	...
	if c.callee is GateAST.GateMember and (c.callee as GateAST.GateMember).safe:
		var sm: GateAST.GateMember = c.callee
	...
	if c.callee is GateAST.GateMember:
		var qm: GateAST.GateMember = c.callee
	...
	if c.callee is GateAST.GateIdent:
		var base: String = (c.callee as GateAST.GateIdent).name
		...
		var mem: GateAST.GateMember = c.callee
		var mangled2: String = _resolve_overload(mem.name, c.args.size())
		if mangled2 != "":
			if mem.target is GateAST.GateSelfExpr:
			...
			if owner != "" and _overload_declared_by_chain(mem.name, owner):
				return "%s.%s(%s)" % [_postfix_base(mem.target), mangled2, ", ".join(args)]
			if owner == "" and mem.target is GateAST.GateIdent:
				var tn: String = (mem.target as GateAST.GateIdent).name
				if _local_types.has(tn) or not _struct_of(tn).is_empty():
					owner = tn
			if owner == "" and mem.target is GateAST.GateIdent and (mem.target as GateAST.GateIdent).name == "super":
				owner = _cur_base
...
func _is_namespace_ref(e) -> bool:
	if not (e is GateAST.GateIdent):
		return false
	var n: String = (e as GateAST.GateIdent).name
	return not _var_types.has(n)


func _emit_soa_call(sname: String, cm: GateAST.GateMember, c: GateAST.GateCall) -> String:
	var info: Dictionary = _soa[sname]
...
func _emit_fstring(fs: GateAST.GateFString) -> String:
	var fmt: String = ""
	var args: PackedStringArray = PackedStringArray()
...
func _discards_safe_call(l: GateAST.GateLambda) -> bool:
	if l.return_type != null and l.return_type.name != "void":
		return false   # the value is used, so the ternary is right
	var e = (l.body[0] as GateAST.GateExprStmt).expr
	if not (e is GateAST.GateCall):
		return false
	var c: GateAST.GateCall = e
	return c.callee is GateAST.GateMember and (c.callee as GateAST.GateMember).safe


func _emit_lambda(l: GateAST.GateLambda) -> String:
	var head: String = "func%s(%s)" % [(" " + l.name) if l.name != "" else "", _params(l.params)]
	if l.return_type != null:
		head += " -> " + _map_type(l.return_type)
	if l.body.size() == 1 and l.body[0] is GateAST.GateReturnStmt:
		var r: GateAST.GateReturnStmt = l.body[0]
		if r.value == null:
	...
	elif (l.body.size() == 1 and l.body[0] is GateAST.GateExprStmt
			and not _discards_safe_call(l)):
		var saved_pending2: PackedStringArray = _pending
		_pending = PackedStringArray()
		var ex: String = _expr((l.body[0] as GateAST.GateExprStmt).expr)
	...
	if not l.body.is_empty() and l.body[l.body.size() - 1] is GateAST.GateMatchStmt:
