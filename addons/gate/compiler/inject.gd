...
static func run(mod: GateAST.Module, diags: GateDiagnostics, super_calls: Dictionary = {},
...
static func _local_traits(members: Array, out: Dictionary) -> void:
	for m in members:
		if m is GateAST.ClassDecl:
			if (m as GateAST.ClassDecl).form == "trait":
				out[m] = true
			_local_traits((m as GateAST.ClassDecl).members, out)
...
static func _reline_foreign_traits(members: Array, traits: Dictionary, local: Dictionary) -> void:
	for m in members:
		...
		for tname in cd.traits:
			...
			if not (decl is GateAST.ClassDecl) or local.has(decl):
				continue
			for i in cd.members.size():
				if _from_trait(decl as GateAST.ClassDecl, cd.members[i]):
					cd.members[i] = _relined(clone(cd.members[i]), cd.line)


static func _from_trait(decl: GateAST.ClassDecl, m) -> bool:
	if decl.members.has(m):
		return true
	if not (m is GateAST.ASTNode):
		return false
	var node: GateAST.ASTNode = m
	for tm in decl.members:
		if tm.get_script() == node.get_script() and (tm as GateAST.ASTNode).line == node.line 				and (tm as GateAST.ASTNode).col == node.col:
			return true
	return false
...
static func _relined(node: Variant, line: int) -> Variant:
	if node is Array:
		for x in node:
			_relined(x, line)
	elif node is GateAST.ASTNode:
		var n: GateAST.ASTNode = node
		n.line = line
		if n is GateAST.Stmt:
			(n as GateAST.Stmt).injected = true
		if n is GateAST.VarDecl:
			(n as GateAST.VarDecl).setter_line = line
		for p in n.get_property_list():
			if int(p["usage"]) & PROPERTY_USAGE_SCRIPT_VARIABLE:
				var v: Variant = n.get(p["name"])
				if v is GateAST.ASTNode or v is Array:
					_relined(v, line)
	return node
...
static func _run_class(owner: Object, members: Array, cls_name: String, super_calls: Dictionary,
	...
	var asserts: Array = []
	var guarded: Array = []    ## [VarDecl, its @export_if]
	for m in members:
		if m is GateAST.ClassDecl:
			var cd: GateAST.ClassDecl = m
			...
			var req: GateAST.Annotation = annotation(vd.annotations, "required")
			if req != null:
				asserts.append(_required_assert(vd, cls_name, req.line, req.col))
			var ei: GateAST.Annotation = annotation(vd.annotations, "export_if")
			if ei != null and ei.args.size() == 1:
				guarded.append([vd, ei])
	if not asserts.is_empty():
		var line: int = (asserts[0] as GateAST.Stmt).line
...
static func _export_if(members: Array, guarded: Array, inherited: bool, diags: GateDiagnostics) -> void:
	...
	for m in members:
		if m is GateAST.VarDecl and not (m as GateAST.VarDecl).is_const:
			props[(m as GateAST.VarDecl).name] = m
	var reads: Dictionary = {}
	for g in guarded:
		for r in condition_reads((g[1] as GateAST.Annotation).args[0]):
			reads[r] = true
	var param: String = "property"
	var own: GateAST.FuncDecl = find_func(members, "_validate_property")
	if own != null and not own.params.is_empty():
		param = (own.params[0] as GateAST.Param).name
	else:
	...
	for g in guarded:
		var vd: GateAST.VarDecl = g[0]
		var ei: GateAST.Annotation = g[1]
		var cond: GateAST.Expr = ei.args[0]
		if reads.has(param):
	...
	var line: int = (stmts[0] as GateAST.Stmt).line
...
static func _through_self(cond: GateAST.Expr, name: String) -> GateAST.Expr:
	if cond is GateAST.Ident and (cond as GateAST.Ident).name == name:
		var s: GateAST.SelfExpr = GateAST.SelfExpr.new()
		...
		return m
	var copy: GateAST.ASTNode = clone(cond)
	for p in copy.get_property_list():
		if not (int(p["usage"]) & PROPERTY_USAGE_SCRIPT_VARIABLE):
			continue
		var v = copy.get(p["name"])
		if v is GateAST.Expr and not (copy is GateAST.Member and p["name"] == "name"):
			copy.set(p["name"], _through_self(v, name))
		elif v is Array:
			var out: Array = []
			for x in v:
				out.append(_through_self(x, name) if x is GateAST.Expr else x)
			copy.set(p["name"], out)
	if copy is GateAST.Call and (cond as GateAST.Call).callee is GateAST.Ident:
		(copy as GateAST.Call).callee = clone((cond as GateAST.Call).callee)
	return copy


static func super_call(args: Array, line: int) -> GateAST.Stmt:
	var callee: GateAST.Ident = GateAST.Ident.new()
	callee.name = "super"
	var call: GateAST.Call = GateAST.Call.new()
	call.callee = callee
	for a in args:
		...
		id.at(line, 1)
		call.args.append(id)
	var st: GateAST.ExprStmt = GateAST.ExprStmt.new()
	st.expr = call
	for n in [callee, call, st]:
		(n as GateAST.ASTNode).at(line, 1)
	return st


## A @tool script runs `_ready` in the editor too, where an unset node is normal.
static func _outside_editor(body: Array, line: int) -> GateAST.Stmt:
	var engine: GateAST.Ident = GateAST.Ident.new()
	engine.name = "Engine"
	var member: GateAST.Member = GateAST.Member.new()
	member.target = engine
	member.name = "is_editor_hint"
	var call: GateAST.Call = GateAST.Call.new()
	call.callee = member
	var neg: GateAST.Unary = GateAST.Unary.new()
	neg.op = "not"
	neg.operand = call
	...
	for n in [engine, member, call, neg, st]:
		(n as GateAST.ASTNode).at(line, 1)
	for s in body:
		(s as GateAST.Stmt).injected = true
	return st


static func _hide_unless(param: String, prop: String, cond: GateAST.Expr, line: int, col: int) -> GateAST.Stmt:
	var p1: GateAST.Ident = GateAST.Ident.new()
	p1.name = param
	var pname: GateAST.Member = GateAST.Member.new()
	pname.target = p1
	pname.name = "name"
	var lit: GateAST.Literal = GateAST.Literal.new()
	lit.kind = "string"
	lit.raw = "\"%s\"" % prop
	var eq: GateAST.Binary = GateAST.Binary.new()
	eq.op = "=="
	eq.left = pname
	eq.right = lit
	var neg: GateAST.Unary = GateAST.Unary.new()
	neg.op = "not"
	neg.operand = cond
	var test: GateAST.Binary = GateAST.Binary.new()
	test.op = "and"
	test.left = eq
	test.right = neg
	var p2: GateAST.Ident = GateAST.Ident.new()
	p2.name = param
	var usage: GateAST.Member = GateAST.Member.new()
	usage.target = p2
	usage.name = "usage"
	var bit: GateAST.Ident = GateAST.Ident.new()
	bit.name = "PROPERTY_USAGE_EDITOR"
	var mask: GateAST.Unary = GateAST.Unary.new()
	mask.op = "~"
	mask.operand = bit
	var assign: GateAST.AssignStmt = GateAST.AssignStmt.new()
	assign.target = usage
	assign.op = "&="
	assign.value = mask
	...
	for n in [p1, pname, lit, eq, neg, test, p2, usage, bit, mask, assign, st]:
		(n as GateAST.ASTNode).at(line, col)
	return st


static func _notify_on_set(members: Array, vd: GateAST.VarDecl, line: int, col: int,
		diags: GateDiagnostics) -> void:
	var fname: String = inline_setter(vd)
	if fname != "":
		var fd: GateAST.FuncDecl = find_func(members, fname)
		if fd == null:
			return   # the checker has reported it
		var mine: GateAST.FuncDecl = _own(members, fd)
		mine.body = notify_every_exit(mine.body, line, col)
		return
	var prop: GateAST.VarDecl = _own(members, vd)
...
static func _notify(line: int, col: int) -> GateAST.Stmt:
	var callee: GateAST.Ident = GateAST.Ident.new()
...
static func notify_every_exit(body: Array, line: int, col: int) -> Array:
	...
	if out.is_empty() or not (out[out.size() - 1] is GateAST.ReturnStmt):
...
static func _before_returns(body: Array, line: int, col: int) -> Array:
	...
	for s in body:
		if s is GateAST.ReturnStmt:
			...
			(s as GateAST.ForStmt).body = _before_returns((s as GateAST.ForStmt).body, line, col)
		elif s is GateAST.WhileStmt:
			(s as GateAST.WhileStmt).body = _before_returns((s as GateAST.WhileStmt).body, line, col)
...
static func _parse_set_block(vd: GateAST.VarDecl) -> bool:
	if vd.set_ast != null:
	...
	var toks: Array[GateLexer.Token] = GateLexer.new().tokenize(body, d, maxi(src_line, 1))
	var mod: GateAST.Module = GateParser.new().parse(toks, src, d)
	if d.has_errors() or mod.members.size() != 1 or not (mod.members[0] is GateAST.FuncDecl):
		return false
	var fd: GateAST.FuncDecl = mod.members[0]
...
static func expand_accessors(members: Array) -> void:
	...
	while i < members.size():
		var m = members[i]
		if m is GateAST.ClassDecl:
			expand_accessors((m as GateAST.ClassDecl).members)
		elif m is GateAST.VarDecl and (m as GateAST.VarDecl).setter != "":
			var vd: GateAST.VarDecl = m
			var made: Array = []
...
static func _parse_accessor(vd: GateAST.VarDecl, kind: String) -> Array:
	var lines: PackedStringArray = vd.setter.split("\n")
	...
	var fd: GateAST.FuncDecl = mod.members[0]
	fd.accessor = true
	if kind == "set" and fd.params.size() == 1 and (fd.params[0] as GateAST.Param).type == null \
			and vd.type != null:
		(fd.params[0] as GateAST.Param).type = vd.type   # Godot types it as the property
...
static func find_func(members: Array, fname: String) -> GateAST.FuncDecl:
	for m in members:
...
static func inline_setter(vd: GateAST.VarDecl) -> String:
...
static func _own(members: Array, node: GateAST.ASTNode) -> GateAST.ASTNode:
	var i: int = members.find(node)
	var copy: GateAST.ASTNode = clone(node)
	if i >= 0:
		members[i] = copy
	return copy
...
static func clone(node):
	...
	if not (node is GateAST.ASTNode):
		return node
	var copy: GateAST.ASTNode = (node as Object).get_script().new()
...
static func _reads(e, out: Array) -> void:
	...
	if e is GateAST.Ident:
		...
		var mb: GateAST.Member = e
		if mb.target is GateAST.SelfExpr:
		...
		_reads((e as GateAST.Binary).left, out)
		_reads((e as GateAST.Binary).right, out)
	elif e is GateAST.Unary:
		_reads((e as GateAST.Unary).operand, out)
	elif e is GateAST.Ternary:
		_reads((e as GateAST.Ternary).cond, out)
		_reads((e as GateAST.Ternary).if_true, out)
		_reads((e as GateAST.Ternary).if_false, out)
	elif e is GateAST.NullCoalesce:
		_reads((e as GateAST.NullCoalesce).left, out)
		_reads((e as GateAST.NullCoalesce).right, out)
	elif e is GateAST.Index:
		_reads((e as GateAST.Index).target, out)
		_reads((e as GateAST.Index).index, out)
	elif e is GateAST.IsExpr:
		_reads((e as GateAST.IsExpr).operand, out)
	elif e is GateAST.CastExpr:
		_reads((e as GateAST.CastExpr).operand, out)
	elif e is GateAST.ArrayLit:
		for x in (e as GateAST.ArrayLit).elements:
			_reads(x, out)
...
static func annotation(annotations: Array, name: String) -> GateAST.Annotation:
...
static func has_export(annotations: Array) -> bool:
	for a in annotations:
		if is_export((a as GateAST.Annotation).name):
			return true
	return false


static func emitted_name(vd: GateAST.VarDecl) -> String:
...
static func prepend_to(cls_members: Array, fname: String, sig: String, stmts: Array, line: int,
	...
	for s in stmts + after:
		(s as GateAST.Stmt).injected = true
	var fd: GateAST.FuncDecl = find_func(cls_members, fname)
	if fd != null:
		var mine: GateAST.FuncDecl = _own(cls_members, fd)
		var body: Array = stmts.duplicate()
		body.append_array(mine.body)
		mine.body = body
		return
	var made: GateAST.FuncDecl = parse_func(sig)
	made.body = stmts + after
	made.injected = true
	made.at(line, 1)
	for p in made.params:
		(p as GateAST.Param).at(line, 1)
	cls_members.append(made)


static func parse_func(sig: String) -> GateAST.FuncDecl:
	var src: String = "func %s:\n\tpass\n" % sig
	var d: GateDiagnostics = GateDiagnostics.new()
	var mod: GateAST.Module = GateParser.new().parse(GateLexer.new().tokenize(src, d), src, d)
	return mod.members[0] as GateAST.FuncDecl


static func _required_assert(vd: GateAST.VarDecl, cls_name: String, line: int, col: int) -> GateAST.Stmt:
	var name: String = emitted_name(vd)
	var target: GateAST.Ident = GateAST.Ident.new()
	target.name = name
	var nul: GateAST.Literal = GateAST.Literal.new()
	nul.kind = "null"
	nul.raw = "null"
	var cmp: GateAST.Binary = GateAST.Binary.new()
	cmp.op = "!="
	cmp.left = target
	cmp.right = nul
	var msg: GateAST.Literal = GateAST.Literal.new()
	msg.kind = "string"
